...
def to_csv_file(filename, headers, rows, delimiter=',', new_line="\n"):
    with open(filename, 'w', encoding='utf8') as output_f:
        list_ = [elem + delimiter if elem != headers[-1] else elem for elem in headers]
        output_f.writelines(list_)
        output_f.write(new_line)
        for list_ in rows:
            list_ = [elem + delimiter if elem != list_[-1] else elem for elem in list_]
            output_f.writelines(list_)
            output_f.write(new_line)
